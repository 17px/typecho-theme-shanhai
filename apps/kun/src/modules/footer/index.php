...
<footer class="mx-auto <?php $this->options->viewWidth() ?> dark:bg-gray-800" role="contentinfo">
    <div class="w-full mx-auto max-w-screen-xl p-2 md:flex md:items-center md:justify-between">
        <div class="flex items-center text-xs text-gray-500 sm:text-center dark:text-gray-400">© <?php echo date('Y'); ?>
            <a href="<?php $this->options->siteUrl(); ?>" class="text-xs  hover:underline pl-2"><?php $this->options->title(); ?></a>
        </div>
        <ul class="flex flex-wrap gap-2 items-center mt-3 font-medium text-gray-500 dark:text-gray-400 sm:mt-0">
            <li>
